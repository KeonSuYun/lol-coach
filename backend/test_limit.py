...
def run_test():
    ...
    try:
        ...
        print(f"   用户身份: {'👑 PRO' if is_pro else '👤 普通用户'}")
        print(f"   R1 深度思考: 上限 {r1_limit} | 已用 {r1_used}")
        
        if is_pro:
            print_color("⚠️ 警告：该用户是 PRO 会员，可能没有次数限制，无法测试拦截！", "yellow")
    ...
    while count <= max_loops:
        ...
        try:
            ...
            if status == 200:
                ...
                # ⚠️ 关键：这里需要 Sleep 3.5秒，因为你的 server.py 有 3秒 的防刷冷却 (ANALYZE_LIMIT_STORE)
                # 如果不 Sleep，会被 429 拦截，无法测试到次数耗尽的 403
                print("   ⏳ 冷却中 (等待 3.5s)...")
                time.sleep(3.5)
                
            elif status == 429:
                print_color("⚠️ 速度太快 (429 Too Many Requests)", "yellow")
                print("   说明：触发了防刷冷却，正在重试...")
                time.sleep(4) # 等久一点
                ...
                if "不足" in err_msg or "限额" in err_msg or "余额" in err_msg:
                    ...
                    print_color("\n⚠️ 触发了 403，但提示信息似乎不是关于次数限制的，请检查。", "yellow")
    ...
    try:
        ...
        print(f"   数据库记录最终已用次数: {final_used}")
    except:
        pass
