...
import { RefreshCw, Lightbulb, Target, Swords, Brain, ShieldAlert, Eye, EyeOff, FileText, Layout, MessageSquarePlus, Copy, Check, X } from 'lucide-react';
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';
import { toast } from 'react-hot-toast';
import { createPortal } from 'react-dom';

// 🛠️ 智能解析器 V2.2 (定点截取版 - 解决 Concise 消失问题)
        ...
        if (detailedStart !== -1) {
            ...
            while ((titleMatch = titleRegex.exec(detailedStr)) !== null) {
                ...
                if (contentMatch) {
                    ...
                    if (endQuoteIdx !== -1) {
                        content = detailedStr.substring(contentStartIdx, endQuoteIdx);
                    } else {
                        content = detailedStr.substring(contentStartIdx);
                    }
...
// 🟢 这里增加了 setFeedbackContent 参数
const AnalysisResult = ({ aiResult, isAnalyzing, setShowFeedbackModal, handleRegenerate, setFeedbackContent }) => {
    ...
    const handleSelectionFeedback = (e) => {
        ...
        if (selectionMenu && selectionMenu.text) {
            navigator.clipboard.writeText(selectionMenu.text).then(() => {
                // ✨ 关键修改：调用父组件的 setFeedbackContent 方法
        ...
        <div className="flex flex-col h-full gap-4 overflow-hidden relative">
            <div className="bg-[#232329]/90 backdrop-blur rounded-xl p-4 border border-white/10 shadow-lg shrink-0 transition-all group relative">
                ...
                <div className="flex items-start gap-4">
                    <div className="p-3 rounded-lg bg-gradient-to-br from-amber-500/20 to-orange-600/20 text-amber-400 border border-amber-500/30 shrink-0 shadow-[0_0_15px_rgba(245,158,11,0.2)]"><Lightbulb size={24} /></div>
